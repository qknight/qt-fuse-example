...
int wrap_read_buf(const char *path, struct fuse_bufvec **bufp, size_t size, off_t off, struct fuse_file_info *fileInfo) {
    return ExampleFS::Instance()->Read_buf(path, bufp, size, off, fileInfo);
}
